...
#ifdef _DEBUG
ofstream http_debug( "http_debug.txt" );
#endif
	...
	)
{
	...
	//::MessageBoxA(NULL,soap_request.c_str(),0,0);
	...
	//::MessageBoxA(NULL,soap_responce.c_str(),0,0);
	...
	)
{
	...
	string soap_args = boost::str( 
		format( AA_GET_ACTIVECONNECTION )
		% connection_id
		);
	string soap_body = boost::str(
		format( SOAP_ENVELOPE )
		% AA_GET_ACTIVECONNECTION
		% service_type.c_str()
		% soap_args.c_str()
		% AA_GET_ACTIVECONNECTION
		);
